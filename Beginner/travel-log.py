country = input("enter country") # Add country name
visits = int(input("enter number of visits")) # Number of visits
list_of_cities = eval(input("enter list of cities in list form")) # create list from formatted string

travel_log = [
  {
    "country": "France",
    "visits": 12,
    "cities": ["Paris", "Lille", "Dijon"]
  },
  {
    "country": "Germany",
    "visits": 5,
    "cities": ["Berlin", "Hamburg", "Stuttgart"]
  },
]
# Do NOT change the code above 👆

# TODO: Write the function that will allow new countries
# to be added to the travel_log. 
# first attempt
# def add_new_country (country_visited, total_visits, citites_visited):
#   travel_log.append({"country": country_visited,
#                     "visits": total_visits,
#                     "cities": citites_visited
#                     })

#  solution
def add_new_country(country_visited, total_visits, citites_visited):
    new_country = {}
    new_country["country"] = country_visited
    new_country["visits"] = total_visits
    new_country["cities"] = citites_visited
    travel_log.append(new_country)

# Do not change the code below 👇
add_new_country(country, visits, list_of_cities)
print(f"I've been to {travel_log[2]['country']} {travel_log[2]['visits']} times.")
print(f"My favourite city was {travel_log[2]['cities'][0]}.")